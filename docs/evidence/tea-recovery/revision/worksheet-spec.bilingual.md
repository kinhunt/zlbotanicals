...
Revision for independent re-review; not publication approval. Three UTF-8 BOM CSVs are delivered, with all numeric cells blank. They are manual records, not a spreadsheet with formulas or an interactive calculator. `schema.json` lists exact columns, numeric fields and permitted template prefills. Python tests use explicitly synthetic numbers only; no factory measurements are supplied.

本修订供独立复审，不代表发布批准。交付三张UTF-8 BOM空白CSV，数值格全部留空。它们是手工记录表，不含自动公式，也不是交互计算器。`schema.json`列出完整字段、数值字段和模板允许预填项。Python测试中的数字仅为合成算例，不是工厂实测结果。
...
Run `python -B validate.py` in this directory. Tests check CSV widths, blank numeric cells, required fields and reference arithmetic on synthetic fixtures. They do not validate a live worksheet calculator, analytical extraction method, factory yield or an entire user-entered dataset. Real samples, uncertainty assessment and independent re-review remain outstanding.

在本目录运行`python -B validate.py`。测试核对CSV列宽、数值留空、必需字段及合成算例的参考运算，不验证在线计算器、分析提取方法、工厂产率或完整用户数据。真实样品、不确定度评估及独立复审尚待完成。
...
Original evidence, citation ledger and independent source checks remain unmodified in `../evidence/`, `../ledger.json`, and `../independent/`; this revision adds no new primary findings. / 原始证据、引用账本及独立来源核查保留在上级目录，本修订不增加新的原始研究发现。
