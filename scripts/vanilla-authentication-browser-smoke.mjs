...
const out = process.env.QA_OUT || '/tmp/vanilla-authentication-qa';
...
try {
  for (const width of [390, 1440, 320]) for (const lang of ['en', 'zh']) for (const javaScriptEnabled of [true, false]) {
    ...
    const route = `${prefix}/resources/blog/vanilla-authentication`;
    ...
    try {
      ...
      assert.equal(await page.locator('link[hreflang="en"]').getAttribute('href'),'https://zlbotanicals.com/resources/blog/vanilla-authentication');
      assert.equal(await page.locator('link[hreflang="zh-CN"]').getAttribute('href'),'https://zlbotanicals.com/zh/resources/blog/vanilla-authentication');
      ...
      for (const id of [1]) {
        ...
        const link = page.locator(`article a[href="#vanilla-auth-ref-${id}"]`).first();
        await link.focus(); await settled(page); await page.keyboard.press('Enter');
        await page.waitForURL(u => u.hash === `#vanilla-auth-ref-${id}`); await settled(page);
        const target = page.locator(`#vanilla-auth-ref-${id}`);
        const box = await target.boundingBox();
        assert.ok(box.y >= 65 && box.y < 900, JSON.stringify(box));
        assert.match(await target.innerText(), /Authentic Aroma/);
        record.citations++;
      }
      for (const source of ['research', 'resources/quality-guides', 'resources/blog']) {
        ...
        assert.equal(await page.locator('.vanilla-authentication-reader').count(),1); record.inbound++;
      }
      record.passed=true;
