...
const out = process.env.QA_OUT || '/tmp/saffron-qa';
...
try {
  for (const width of [390, 1440, 320]) for (const lang of ['en', 'zh']) for (const javaScriptEnabled of [true, false]) {
    ...
    const route = `${prefix}/resources/blog/saffron-identity`;
    ...
    try {
      ...
      const metrics = await page.evaluate(() => {
        const table = document.querySelector('.saffron-table');
      ...
      await page.locator('.saffron-table').scrollIntoViewIfNeeded(); await settled(page);
      await page.screenshot({path:`${out}/${key}-comparison-viewport.png`});
      await page.locator('.saffron-table').screenshot({path:`${out}/${key}-comparison.png`});
      for (const id of [1,2]) {
        await page.goto(base+route); await page.evaluate(() => document.fonts.ready);
        const link = page.locator(`article a[href="#saffron-ref-${id}"]`).first();
        await link.focus(); await settled(page); await page.keyboard.press('Enter');
        await page.waitForURL(u => u.hash === `#saffron-ref-${id}`); await settled(page);
        const target = page.locator(`#saffron-ref-${id}`);
        const box = await target.boundingBox();
        assert.ok(box.y >= 65 && box.y < 900, JSON.stringify(box));
        assert.match(await target.innerText(), id===1 ? /Detection of botanical adulterants/ : /Multi-Analytical Approach/);
        record.citations++;
      }
      for (const source of ['quality', 'products/botanical-extracts']) {
        ...
        assert.equal(await page.locator('.saffron-table').count(),1); record.inbound++;
      }
      record.passed=true;
