...
try {
 for(const width of [390,1024,1280,1440]) for(const lang of ['en','zh']){
  ...
  await video.evaluate(v=>{v.muted=false;return v.play();});
  ...
  assert.ok(Math.abs(meta.duration-timing.duration)<0.1);assert.equal(meta.width,1280);assert.equal(meta.height,720);
