...
const out = process.env.QA_OUT || '/tmp/beetroot-qa';
...
try {
  const jobs=[]; for (const kind of ["beetroot-product-formats", "beet-colour-plant-based-meat"]) for (const width of [390,1440,320]) for (const lang of ['en','zh']) for (const javaScriptEnabled of [true,false]) jobs.push({kind,width,lang,javaScriptEnabled});
  async function run({kind,width,lang,javaScriptEnabled}) {
    ...
    try {
      ...
      for(let i=0;i<await cells.count();i++) {
        ...
        assert.ok(aria.includes(value),aria);
      ...
      for(const row of await page.locator('article tbody th').all()) {
       const name=(await row.innerText()).trim();const aria=await row.ariaSnapshot();assert.ok(aria.includes(name));record.rowHeaders.push({name,aria});
      }
      const caption=page.locator('article caption');const capbox=await caption.boundingBox();const tablebox=await page.locator('article table').boundingBox();
      ...
      for (const id of (kind==='beetroot-product-formats'?[1,2,3,4]:[23,28,30,31])) {
        await page.goto(base+route); await page.evaluate(() => document.fonts.ready);
      ...
      for (const source of ['research', 'solutions/food', 'resources/application-guides', 'resources/blog']) {
        ...
        assert.equal(await page.locator(`.beetroot-reader`).count(),1); record.inbound++;
      }
      record.passed=true;
