...
const out = process.env.QA_OUT || '/tmp/olive-pectin-qa';
...
try {
  const jobs=[]; for (const kind of ["citrus-pectin-gel-selection", "olive-leaf-beverage-encapsulation"]) for (const width of [390,1440,320]) for (const lang of ['en','zh']) for (const javaScriptEnabled of [true,false]) jobs.push({kind,width,lang,javaScriptEnabled});
  async function run({kind,width,lang,javaScriptEnabled}) {
    ...
    try {
      ...
      assert.equal(metrics.tables,kind.startsWith('olive')?0:1); if(!kind.startsWith('olive')) assert.ok(metrics.cards>0); assert.ok(parseFloat(metrics.font)>=16);
      ...
      for (const id of (kind==='citrus-pectin-gel-selection'?[4,6,11,12,13]:[4,5,7,8,9])) {
        await page.goto(base+route); await page.evaluate(() => document.fonts.ready);
      ...
      for (const source of ['research', 'solutions/beverages', 'solutions/food', 'resources/application-guides', 'resources/blog']) {
        ...
        assert.equal(await page.locator(`.olive-pectin-reader`).count(),1); record.inbound++;
      }
      record.passed=true;
