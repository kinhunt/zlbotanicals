...
const out = process.env.QA_OUT || '/tmp/vanilla-qa';
...
try {
  for (const width of [390, 1440, 320]) for (const lang of ['en', 'zh']) for (const javaScriptEnabled of [true, false]) {
    ...
    const route = `${prefix}/resources/blog/vanilla-material-choice`;
    ...
    try {
      ...
      const allMetrics = [];
      for (let tableIndex=0; tableIndex<1; tableIndex++) {
      const metrics = await page.evaluate((tableIndex) => {
        const table = document.querySelectorAll('.vanilla-table')[tableIndex];
      ...
      const makerLinks=await page.locator('.vanilla-table th[scope="row"] a').evaluateAll(els=>els.map(el=>el.href));
      assert.deepEqual(makerLinks,['https://nielsenmassey.com/products/pure-vanilla-powder-bulk-sizes','https://www.singingdogvanilla.com/product/organic-ground-vanilla-bean-grinder','https://nielsenmassey.com/products/pure-vanilla-bean-paste']);
      ...
      if(javaScriptEnabled && width!==390) {
       const table=page.locator('.vanilla-table');
       await table.locator('caption').evaluate(el=>el.scrollIntoView({block:'start',behavior:'instant'}));
       await page.evaluate(()=>scrollBy(0,-110));
       await page.screenshot({path:`${out}/${key}-caption.jpg`,type:'jpeg',quality:75});
       if(width===1440) await table.screenshot({path:`${out}/${key}-table.jpg`,type:'jpeg',quality:75});
       if(width===320) for(let row=0;row<3;row++) {
        await table.locator('tbody tr').nth(row).evaluate(el=>el.scrollIntoView({block:'start',behavior:'instant'}));
        await page.evaluate(()=>scrollBy(0,-110));
        await page.screenshot({path:`${out}/${key}-row${row}.jpg`,type:'jpeg',quality:75});
       }
      }
      for (const id of [1,2,3,4]) {
        await page.goto(base+route); await page.evaluate(() => document.fonts.ready);
        const link = page.locator(`article a[href="#vanilla-ref-${id}"]`).first();
        await link.focus(); await settled(page); await page.keyboard.press('Enter');
        await page.waitForURL(u => u.hash === `#vanilla-ref-${id}`); await settled(page);
        const target = page.locator(`#vanilla-ref-${id}`);
        const box = await target.boundingBox();
        assert.ok(box.y >= 65 && box.y < 900, JSON.stringify(box));
        assert.match(await target.innerText(), id===1 ? /Pure Vanilla Powder/ : id===2 ? /Grinder/ : id===3 ? /eCFR/ : /Pure Vanilla Bean Paste/);
        record.citations++;
      }
      for (const source of ['research', 'resources/quality-guides', 'resources/blog']) {
        ...
        assert.equal(await page.locator('.vanilla-table').count(),1); record.inbound++;
      }
      record.passed=true;
