...
const out = process.env.QA_OUT || '/tmp/cranberry-qa';
...
try {
  for (const width of [390, 1440, 320]) for (const lang of ['en', 'zh']) for (const javaScriptEnabled of [true, false]) {
    ...
    const route = `${prefix}/resources/blog/cranberry-pac-purchasing`;
    ...
    try {
      ...
      const metrics = await page.evaluate((tableIndex) => {
        const table = document.querySelectorAll('.cranberry-table')[tableIndex];
      ...
      const exactRows=await page.locator('.cranberry-table tbody tr').evaluateAll(rows=>rows.map(row=>[row.cells[0].textContent,...[...row.querySelectorAll('.table-value')].map(x=>x.textContent)]));
      assert.deepEqual(exactRows,[['#4','36','34.86 ± 1.73','96.83%'],['#6','36','4.91 ± 0.22','13.64%'],['#24','36','37.17 ± 1.53','103.25%']]);
      ...
      await page.locator('.cranberry-table').first().scrollIntoViewIfNeeded(); await settled(page);
      await page.screenshot({path:`${out}/${key}-comparison-viewport.png`});
      for(let i=0;i<1;i++) {
       const table=page.locator('.cranberry-table').nth(i);
      ...
      for (const id of [1,2,4,6]) {
        ...
        const link = page.locator(`article a[href="#cranberry-ref-${id}"]`).first();
        await link.focus(); await settled(page); await page.keyboard.press('Enter');
        await page.waitForURL(u => u.hash === `#cranberry-ref-${id}`); await settled(page);
        const target = page.locator(`#cranberry-ref-${id}`);
        const box = await target.boundingBox();
        assert.ok(box.y >= 65 && box.y < 900, JSON.stringify(box));
        assert.match(await target.innerText(), id===1 ? /USDA/ : id===2 ? /2020 cranberry/ : id===4 ? /validation/ : /2026-europepmc/);
        record.citations++;
      }
      for (const source of ['research', 'quality', 'resources/quality-guides', 'resources/blog']) {
        ...
        assert.equal(await page.locator('.cranberry-table').count(),1); record.inbound++;
      }
      record.passed=true;
