...
const out = process.env.QA_OUT || '/tmp/butterfly-monkfruit-qa';
...
try {
  const jobs=[]; for (const kind of ["monk-fruit-specification-numbers", "butterfly-pea-protein-beverages"]) for (const width of [390,1440,320]) for (const lang of ['en','zh']) for (const javaScriptEnabled of [true,false]) jobs.push({kind,width,lang,javaScriptEnabled});
  async function run({kind,width,lang,javaScriptEnabled}) {
    ...
    try {
      ...
      assert.equal(metrics.tables,kind.startsWith('butterfly')?0:2); if(!kind.startsWith('butterfly')) assert.ok(metrics.cards>0); assert.ok(parseFloat(metrics.font)>=16);
      ...
      for (const id of (kind==='monk-fruit-specification-numbers'?[4,7,9,10,11,12]:[1,7,8,9])) {
        await page.goto(base+route); await page.evaluate(() => document.fonts.ready);
      ...
      for (const source of ['research', 'solutions/beverages', 'resources/application-guides', 'resources/blog']) {
        ...
        assert.equal(await page.locator(`.botanical-comparison-reader`).count(),1); record.inbound++;
      }
      record.passed=true;
