...
try {
  const jobs=[]; for (const kind of ['cocoa','ginger']) for (const width of [390,1440,320]) for (const lang of ['en','zh']) for (const javaScriptEnabled of [true,false]) jobs.push({kind,width,lang,javaScriptEnabled});
  async function run({kind,width,lang,javaScriptEnabled}) {
    ...
    try {
      ...
      assert.equal(metrics.tables,1); assert.ok(metrics.cards>0); assert.ok(parseFloat(metrics.font)>=16);
      ...
      const cells=page.locator('article tbody td'); record.accessibleCells=0;
      for(let i=0;i<await cells.count();i++) {
        const cell=cells.nth(i), label=await cell.locator('.mobile-label').innerText();
      ...
      await page.locator('article table').screenshot({path:`${out}/${key}-table.jpg`,type:'jpeg',quality:80});
      for (const id of Array.from({length:kind==='cocoa'?5:4},(_,i)=>i+1)) {
      ...
      for (const source of ['research', 'solutions/beverages', 'solutions/food', 'resources/application-guides', 'resources/blog']) {
        ...
        assert.equal(await page.locator(`.${kind}-reader`).count(),1); record.inbound++;
      }
      record.passed=true;
  ...
  await Promise.all(Array.from({length:4},async()=>{while(jobs.length) await run(jobs.shift());}));
