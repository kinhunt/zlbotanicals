...
const out = process.env.QA_OUT || '/tmp/cocoa-ginger-qa';
...
try {
  for (const kind of ['cocoa','ginger']) for (const width of [390, 1440, 320]) for (const lang of ['en', 'zh']) for (const javaScriptEnabled of [true, false]) {
    ...
    const route = `${prefix}/resources/blog/${kind}-selection`;
    ...
    try {
      ...
      assert.equal(metrics.tables,1); assert.equal(metrics.cards,kind==='cocoa'?8:6); assert.ok(parseFloat(metrics.font)>=16);
      ...
      assert.equal(await page.locator('link[hreflang="en"]').getAttribute('href'),`https://zlbotanicals.com/resources/blog/${kind}-selection`);
      assert.equal(await page.locator('link[hreflang="zh-CN"]').getAttribute('href'),`https://zlbotanicals.com/zh/resources/blog/${kind}-selection`);
      {
      ...
      await table.evaluate(el=>scrollTo({top:el.getBoundingClientRect().top+scrollY-100,behavior:'instant'}));
      await page.screenshot({path:`${out}/${key}-table-viewport.jpg`,type:'jpeg',quality:80});
      await table.screenshot({path:`${out}/${key}-table.png`});
      const rows=table.locator('tbody tr');
      record.accessibleCells=0;
      for(let r=0;r<await rows.count();r++) {
        const row=rows.nth(r);
        await row.evaluate(el=>scrollTo({top:el.getBoundingClientRect().top+scrollY-100,behavior:'instant'}));
        await page.screenshot({path:`${out}/${key}-row${r}-viewport.jpg`,type:'jpeg',quality:80});
        const cells=row.locator('td');
        for(let c=0;c<await cells.count();c++) {
          const cell=cells.nth(c); const label=await cell.locator('.mobile-label').innerText();
          const aria=await cell.ariaSnapshot();
          if(width<=640) { assert.ok(aria.includes(label),aria); record.accessibleCells++; }
          else assert.equal(await cell.locator('.mobile-label').isVisible(),false);
          const rect=await cell.boundingBox(); assert.ok(rect.x>=0 && rect.x+rect.width<=width+1);
        }
      }
      const captionBox=await table.locator('caption').boundingBox();assert.ok(captionBox.width>=(await table.boundingBox()).width*0.95,JSON.stringify(captionBox));
      for (const id of (kind==='cocoa'?[1,2,3,4,5]:[1,2,3,4])) {
        ...
        assert.ok((await target.innerText()).length>20); await page.screenshot({path:`${out}/${key}-citation${id}.jpg`,type:'jpeg',quality:65});
        record.citations++;
      }
      for (const source of ['research', 'solutions/food', 'solutions/beverages', 'resources/application-guides', 'resources/blog']) {
        ...
        assert.equal(await page.locator(`.${kind}-reader`).count(),1); record.inbound++; await page.screenshot({path:`${out}/${key}-inbound-${source.replaceAll('/','-')}.jpg`,type:'jpeg',quality:55});
      }
      record.passed=true;
