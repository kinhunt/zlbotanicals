...
test('all product pages put procurement confirmation and actions before technical reading',()=>{
 ...
 if(slug==='turmeric') {
  ...
  } else { assert.ok(page.includes(`data-extract-sales="${slug}"`)); assert.match(panel,prefix?/振隆(?:面向品牌商及配方制造企业批量)?供应/:/ZL Botanicals supplies/); assert.ok(page.includes(prefix?'书面报价':'confirmed in writing')); }
 }
});
