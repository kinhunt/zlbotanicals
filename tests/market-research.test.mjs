...
for (const [prefix, title] of [['', 'Global mushroom supplements market forecast to reach $8.83 billion by 2031'], ['zh/', '全球蘑菇补充剂市场预计2031年达88.3亿美元']]) {
  test(`${prefix || 'en'} market article has a distinct, discoverable, attributed reading journey`, () => {
    ...
    for (const number of ['8.66%', '75.8%', '14.4%', '114.5%', '4/18', '6/18']) assert.ok(article.includes(number), number);
    assert.match(article, /Mordor Intelligence/);
    assert.match(article, prefix ? /预计|预测/ : /forecast/);
    assert.doesNotMatch(article.replace(/<[^>]+>/g, ""), /\bgoji\b|枸杞/i);
    for (let id=1; id<=30; id++) {
      assert.ok(article.includes(`href="#market-v4-ref-${id}"`), `clickable citation ${id}`);
      assert.ok(article.includes(`id="market-v4-ref-${id}"`), `reference ${id}`);
    }
