...
test('bounded tea serialization preserves every pre-existing sitemap entry byte-for-byte', () => {
  ...
  const existing = entries.filter(entry => !teaUrl.test(loc(entry)) && !/\/resources\/blog\/(?:acacia-fibre-emulsification|citrus-delayed-bitterness|astaxanthin-beverage-stability|chicory-fibre-selection|vanilla-authentication|vanilla-material-choice|cranberry-pac-purchasing|hesperidin-material-choice|hibiscus-color-endpoints|saffron-identity|elderberry-material-selection|paprika-extract-format-selection|ginseng-heat-bitterness|uk-sdil-milk-tea-2028)\/?$/.test(loc(entry)));
