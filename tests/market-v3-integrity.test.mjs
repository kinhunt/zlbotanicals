...
for(const lang of ['en','zh'])test(`${lang}: v3 rendered IDs and markdown are clean`,()=>{const h=read(`dist/${lang==='zh'?'zh/':''}research/market/functional-mushrooms/index.html`),ids=[...h.matchAll(/\bid="([^"]+)"/g)].map(m=>m[1]);assert.equal(ids.length,new Set(ids).size);assert.doesNotMatch(h.replace(/<[^>]+>/g,''),/\*\*|\|---/);assert.match(h,lang==='zh'?/冻干/:/freeze-drying/);assert.ok(h.includes('id="method"'));});
