...
test('thirty-three topics retain all legacy guides and twelve aligned science profiles',()=>{
 ...
 for(const t of data.topics) {
  ...
  const science=profiles.find(p=>p.id===t.scienceId);
...
test('navigation retains encyclopedia entry and its hub exposes all seven branches',()=>{
 for(const [prefix,label] of [['','Plant Extracts'],['zh/','植物提取物']]){
  ...
  const hub=readFileSync(`dist/${prefix}plant-extracts/index.html`,'utf8');
  for(const s of data.sections) assert.ok(hub.includes(`href="/${prefix}plant-extracts/${s.slug}"`));
 }
});
