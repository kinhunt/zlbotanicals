...
test('buyer guides replace unsupported market reports and use the actual update date',()=>{
 const all=entries('blog'); assert.equal(all.length,22);
 const pages=all.filter(({file})=>!['paprika-extract-format-selection.md','uk-sdil-milk-tea-2028.md','ginseng-heat-bitterness.md','tea-haze-diagnosis.md','stevia-temporal-sensory.md','centella-cold-process-stability.md'].includes(file)); assert.equal(pages.length,10);
 for(const {lang,file,text} of pages){
